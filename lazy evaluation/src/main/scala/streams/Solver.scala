...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] =
    (for {
      tuple <- b.legalNeighbors
    } yield (tuple._1, tuple._2 :: history)).toStream
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    neighbors.dropWhile(neighbor => explored.contains(neighbor._1))
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if(initial.isEmpty) Stream.empty
      ...
      val more = for{
        tuple <- initial
        nextPath <- newNeighborsOnly(neighborsWithHistory(tuple._1,tuple._2),explored)
      } yield nextPath
